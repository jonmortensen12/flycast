...
/* ── RUNNING PART OF IT ────────────────────────────────────────────────
   `node smoke.mjs` runs everything, exactly as before. `node smoke.mjs fight
   zone` runs only those sections, which is the difference between a twelve
   minute wait and a few seconds when the change under test touches one thing.
   `node smoke.mjs --list` prints the names.

   READ THIS BEFORE TRUSTING A FILTERED RUN. These tests deliberately share one
   world: they hook fish, spook them, press buttons, move the rig and draw from
   the same random stream, and section 5 of the handoff records a whole bug
   class that comes from one check standing in front of another. A section run
   on its own therefore sees a DIFFERENT world from the same section run in
   sequence, and can pass alone while failing in place, or the reverse. It is
   for a fast answer while you work. It is not a green build, it does not
   print OK, and nothing should ship on one.

   A skipped section executes NOTHING — not the simulation and not the
   reporting. An earlier attempt let the reporting run against a stub result,
   on the reasoning that a stub could be shaped to look like a pass; it cannot,
...
if(ARGS.includes('--list')){
  ...
  console.log('  node smoke.mjs              everything (the only run that can pass)');
  console.log('  node smoke.mjs fight zone   just those');
  process.exit(0);
}
{
  const bogus=ONLY.filter(a=>!SECTIONS.includes(a));
  if(bogus.length){
    console.log('no such section: '+bogus.join(', '));
    console.log('sections: '+SECTIONS.join(' '));
    process.exit(2);
  }
}
...
/* THE SETTLE IS THE FLOOR OF EVERY RUN, so only the section that measures it
   pays for the whole thing. 200 frames of full physics is about 18 seconds and
   exists so `water` can read a converged grid; everything after it only needs
   the world to be roughly right, and 30 frames gives that. Keeping a short
   settle rather than skipping it outright is deliberate: the state these
   frames leave is what later sections start from, and dropping it entirely
   would move their world further than filtering already does. */
for(let f=0;f<(ran('water')?200:30);f++) loop2();
if(ran('water')){
...
for(let f=0;f<(ran('specks')?250:30);f++) loop4();   /* same trade as above */
if(ran('specks')){
...
if(ran('fight')){
  const reeldrag=vm.runInContext(`(()=>{
    ...
    const fight=(size,squeeze)=>{
      ...
      let pk=0;
      /* 400, not 900. This block was four twenty-second fights and cost 238 s
         of a 692 s suite — a third of every run in the project, for four
         numbers. What it asserts is the tension CEILING the squeeze sets,
         which the drag reaches in the first seconds and then holds, and that
         winding gains line, which is monotonic. Nine hundred frames measured
         the same ceiling for twice as long. */
      for(let i=0;i<400&&hooked;i++){ renderer._loop(); pk=Math.max(pk,M.tension); }
      rodPad.buttons[0].value=0; rodPad.buttons[0].pressed=false;
...
if(ONLY.length){
  const skipped=SECTIONS.filter(x=>!ran(x));
  console.log('\nPARTIAL: ran '+ONLY.join(', ')+'  |  skipped '+skipped.join(', '));
  console.log('These tests share one world and the skipped ones move it, so a FAILURE');
  console.log('above may be the filter rather than the code — ROD-BEND, for one, needs');
